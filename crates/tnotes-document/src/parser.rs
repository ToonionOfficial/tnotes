...
use crate::model::{Block, BlockKind, Document, ListItem, Marks, RichText, Span, SubList, TaskItem};
    ...
    fn parse_top_level_handle(
        ...
        match node {
            ...
            Node::Raw(bytes) => {
                ...
                if !text.is_empty() {
                    blocks.push(Block::new(BlockKind::Paragraph(RichText {
                        spans: vec![Span {
                            text,
                            marks: Marks::default(),
                            link: None,
        ...
        match tag_name.as_ref() {
            "p" => {
                ...
                if self.strip_empty_blocks && content.spans.is_empty() {
                    ...
                    Ok(Some(Block::new(BlockKind::Paragraph(content))))
                }
            }
            "h1" | "h2" | "h3" | "h4" | "h5" | "h6" => {
                let level = tag_name[1..].parse::<u8>().unwrap_or(1);
                let content = self.parse_rich_text(tag, dom);
                Ok(Some(Block::new(BlockKind::Heading { level, content })))
            }
            "blockquote" => {
                let content = self.parse_rich_text(tag, dom);
                Ok(Some(Block::new(BlockKind::Quote(content))))
            }
            "pre" => {
                let (language, code) = self.parse_code_block(tag, dom);
                Ok(Some(Block::new(BlockKind::CodeBlock { language, code })))
            }
            "hr" => Ok(Some(Block::new(BlockKind::Divider))),
            "ul" => {
                ...
                if is_task_list {
                    Ok(Some(Block::new(BlockKind::TaskList(
                        self.parse_task_items(tag, dom)?,
                    ))))
                } else {
                    Ok(Some(Block::new(BlockKind::BulletList(
                        self.parse_list_items(tag, dom)?,
                    ))))
                }
            }
            "ol" => Ok(Some(Block::new(BlockKind::OrderedList(
                self.parse_list_items(tag, dom)?,
            )))),
            unsupported => {
                if self.strict_mode {
...
mod tests {
    ...
    fn test_parse_task_list() {
        ...
        assert_eq!(doc.blocks.len(), 1);
        if let BlockKind::TaskList(items) = &doc.blocks[0].kind {
            assert_eq!(items.len(), 2);
    ...
    fn test_mobile_tiptap_multiline_blockquote() {
        ...
        assert_eq!(doc.blocks.len(), 1);
        if let BlockKind::Quote(rt) = &doc.blocks[0].kind {
            assert_eq!(rt.spans.len(), 1);
            assert_eq!(
                rt.spans[0].text,
