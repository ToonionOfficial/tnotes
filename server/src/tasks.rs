...
pub fn run_housekeeping(conn: &Connection) -> (usize, usize) {
    ...
    if expired_sessions > 0 {
        tracing::info!("Housekeeping: cleaned up {} expired sessions", expired_sessions);
    }
    if purged_notes > 0 {
        tracing::info!("Housekeeping: purged {} trashed notes older than 30 days", purged_notes);
    }

    (expired_sessions, purged_notes)
}

pub fn start_housekeeping_task(db: Arc<Mutex<Connection>>, interval_secs: u64) -> tokio::task::JoinHandle<()> {
