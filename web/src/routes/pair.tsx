...
import type { MeResponse, PairingDataResponse, PairStatusResponse } from '@/lib/types'
...
function PairPage() {
  ...
  // Realtime pair detection polling
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        const res = await apiFetch<PairStatusResponse>(
          `/api/pair/status?code=${pairData.pairing_code}&device_id=${pairData.device_id}`,
        )
        if (mounted && res.paired) {
          setPairedDevice(res.device_name || 'Mobile App')
  ...
  }, [pairData.pairing_code, pairData.device_id, pairedDevice, navigate])
  ...
  return (
    <div className="flex min-h-screen items-center justify-center p-4 sm:p-6 bg-background">
      <Card className="w-full max-w-lg border-border/80 shadow-2xl transition-all duration-300">
        <CardHeader className="text-center pb-3">
          ...
          <CardDescription className="text-sm">
            {pairedDevice
              ? `Connected to ${pairedDevice}. Redirecting to your vault…`
              : isWelcome
        ...
        <CardContent className="space-y-5">
          {pairedDevice ? (
            <div className="flex flex-col items-center justify-center rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-8 text-center space-y-3 animate-in fade-in zoom-in-95 duration-300">
              <div className="flex h-16 w-16 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-400">
                <Check className="h-8 w-8 stroke-[3]" />
              </div>
              <div>
                <h3 className="text-lg font-bold text-foreground">Synced & Ready</h3>
                <p className="text-xs text-muted-foreground pt-1">
                  Your notes and folders are now synchronized across your devices.
                </p>
              </div>
            ...
            <>
              ...
              <div className="rounded-xl bg-muted/40 border border-border/50 p-3.5 text-xs text-muted-foreground space-y-1.5">
                ...
                <ol className="list-decimal list-inside space-y-1 pl-1 text-[11px] leading-relaxed">
                  ...
                  <li>
                    Open <strong>Settings</strong> $\rightarrow$ tap{' '}
          ...
          <div className="space-y-2 pt-1">
            <Button
              ...
              size="lg"
            >
              {pairedDevice ? 'Open My Vault Now' : isWelcome ? 'Open My Vault' : 'Done'}
            </Button>
          </div>
        </CardContent>
      </Card>
