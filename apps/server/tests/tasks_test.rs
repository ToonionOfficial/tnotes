...
async fn test_housekeeping_task() {
    ...
    let mut recent_trashed = Note::new("Recent Note", "Body", "Recent Searchable", None, "device_active", &user.id);
    recent_trashed.trashed = true;
    recent_trashed.deleted_at = Some(now - (5 * ms_per_day));

    let mut old_trashed = Note::new("Old Note", "Body", "Old Searchable", None, "device_active", &user.id);
