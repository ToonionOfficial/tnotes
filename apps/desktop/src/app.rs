...
pub fn run_app() {
    let db = AppDb::init().unwrap_or_else(|err| {
        eprintln!(
            "Error initializing local SQLite database: {err}. Falling back to in-memory mode."
        );
    ...
    Application::with_platform(platform)
        ...
        .run(move |cx: &mut App| {
            ...
            let auth_store =
                cx.new(|_cx| AuthStore::new(db.default_user_id.clone(), db.device_id.clone()));
            let sync_store = cx.new(|_cx| SyncStore::new());

            let app_state =
                cx.new(|_cx| AppState::new(note_store, folder_store, auth_store, sync_store));
